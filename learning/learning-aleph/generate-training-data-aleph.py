...
def get_aleph_parameters_and_command(options):
...
# The following parameters are concerned with testing theories: test_pos, test_neg, train_pos, train_neg.
        ...
        aleph_parameters = {'clauselength' : '5',
                     'lookahead' : '2',
                     'evalfn' : 'coverage',
                     'mingain' : '0.01',
                     'minacc' : '0',
...
#                     'verbosity' : '0',
        ...
        aleph_parameters['minpos'], aleph_parameters['noise']  = (100, 0) if options.learn_bad else (10,10)

    return aleph_parameters, aleph_command


def write_yap_file(filename_path, YAP_PATH, ALEPH_PATH, EXAMPLES_PATH, HYPOTHESIS_FILE, aleph_parameters, aleph_command):

    yap_script_template = """
#!/bin/bash

ALEPH_PATH={ALEPH_PATH}
YAP_PATH={YAP_PATH}
...
if __name__ == "__main__":
    ...
    argparser.add_argument("--all-ops-file", default="all_operators.bz2", help="path for finding all operators")
    ...
    if not os.path.exists(options.store_training_data):
        ...
        aleph_directory =  options.aleph_directory if options.aleph_directory else f'{os.path.dirname(__file__)}/../aleph'

        assert os.path.isfile(f"{aleph_directory}/aleph.pl"), "Error: aleph not found at {}/aleph.pl".format(aleph_directory)
        shutil.copy (f"{aleph_directory}/aleph.pl", options.store_training_data)
    ...
    #         aleph_parameters, aleph_command = get_aleph_parameters_and_command()
    #         write_yap_file (os.path.join(options.store_training_data, "learn-" + filename),
    #                         options.yap, options.store_training_data,
    #                         filename, aleph_parameters, aleph_command)
    ...
    aleph_parameters, aleph_command = get_aleph_parameters_and_command(options)
    # write the actual files
    for schema in action_schemas:
        filename = schema.name
        filename_path = os.path.join(options.store_training_data, filename)

        write_yap_file (os.path.join(options.store_training_data, "learn-" + filename), options.yap, options.store_training_data, filename, filename + ".h", aleph_parameters, aleph_command)
